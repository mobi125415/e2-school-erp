...
def data_root():
    """Return a writable data directory for desktop and serverless deployments.

    Vercel's deployed filesystem is read-only except for /tmp. Persistent school
    data should therefore live in DATABASE_URL (PostgreSQL) in production.
    SQLite is retained as a local/development fallback and uses /tmp on Vercel.
    """
    if os.environ.get("VERCEL") or os.environ.get("AWS_LAMBDA_FUNCTION_NAME"):
        base = Path(os.environ.get("E2_RUNTIME_DIR", "/tmp"))
...
# A random SECRET_KEY on every serverless cold start invalidates Flask session
# cookies. Always configure a stable secret in production, with a safe local
# fallback for desktop development.
SECRET_KEY = (
    os.environ.get("SECRET_KEY")
    or os.environ.get("E2_SECRET_KEY")
    or "e2-local-development-secret-change-me"
)
app.config["SECRET_KEY"] = SECRET_KEY

# Accept the common Vercel/Neon/Postgres environment variable names. This keeps
# the original app usable locally while allowing school accounts to persist
# across Vercel function invocations when a PostgreSQL integration is attached.
DATABASE_URL = (
...
if os.environ.get("VERCEL") or os.environ.get("E2_ENV", "").lower() == "production":
    app.config.update(
...
def migrate_institute_branding():
    # PRAGMA is SQLite-only. PostgreSQL already receives the complete schema
    # through db.create_all(), so do not run SQLite SQL against it.
    if db.engine.dialect.name != "sqlite":
        return
    try:
        ...
        db.session.rollback()
        print("[E2] Institute branding migration skipped:", repr(exc))
...
def master():
    ...
    if request.method=="POST":
        try:
            ...
            db.session.rollback()
            print("[E2] Master create institute failed:", repr(exc))
            flash("Unable to create the school. Check the email/password and database configuration.","danger")
...
def migrate_erp22_content_bank():
    ...
    # PostgreSQL gets the current columns from SQLAlchemy db.create_all().
    if db.engine.dialect.name != "sqlite":
        return
    try:
