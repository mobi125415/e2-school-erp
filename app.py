...
@app.route("/healthz")
def healthz():
    """Deployment diagnostic that never touches application pages."""
    try:
        ...
        db.session.rollback()
        print("[E2] /healthz database error:", repr(exc))
        return jsonify({
            "ok": False,
...
# On Vercel this is normally /tmp and there is no bundled writable database.
recover_legacy_database(DB_PATH)

# Keep the Flask module importable even when a cloud database has a temporary
# connectivity/configuration problem. Requests will expose the database error
# through /healthz instead of Vercel reporting only FUNCTION_INVOCATION_FAILED.
...
try:
    ...
    _STARTUP_DB_ERROR = exc
    try:
        db.session.rollback()
    except Exception:
        pass
    print("[E2] Database startup failed:", repr(exc))
