...
application = app
# Trust the standard single reverse proxy used by managed cloud hosts.
...
if DATABASE_URL:
    ...
    if os.environ.get("VERCEL") or os.environ.get("NOW_REGION"):
        print("[E2] WARNING: Vercel is running without DATABASE_URL/POSTGRES_URL. "
              "SQLite under /tmp is temporary and is NOT a persistent production database.")
...
@app.after_request
def cloud_api_headers(response):
    # The mobile/web clients use the same HTTPS API. Keep CORS narrow by
    # allowing configured origins only; "*" can be enabled explicitly for
    # initial testing with E2_CORS_ORIGINS=*.
    origins = os.environ.get("E2_CORS_ORIGINS", "").strip()
    if request.path.startswith("/api/") and origins:
        if origins == "*":
            response.headers["Access-Control-Allow-Origin"] = "*"
        else:
            origin = request.headers.get("Origin", "")
            allowed = {x.strip() for x in origins.split(",") if x.strip()}
...
def migrate_db():
    """Small migration for SQLite; PostgreSQL uses create_all and does not need SQLite PRAGMA migrations."""
    try:
        if db.engine.dialect.name != "sqlite":
            print(f"[ERP] {db.engine.dialect.name} database detected; skipping SQLite-only migrations.")
            return
        cols=[r[1] for r in db.session.execute(db.text("PRAGMA table_info(student)")).fetchall()]
        if "books_copies_amount" not in cols:
            db.session.execute(db.text("ALTER TABLE student ADD COLUMN books_copies_amount FLOAT DEFAULT 0"))
        if "photo" not in cols:
            db.session.execute(db.text("ALTER TABLE student ADD COLUMN photo VARCHAR(255) DEFAULT ''"))
        db.session.commit()
    except Exception:
        db.session.rollback()
...
def current_inst():
    ...
    if not iid:
        return None
    try:
        return db.session.get(Institute, iid)
    except Exception as exc:
        # A broken/empty database must never make public pages such as
        # /login return HTTP 500. The protected pages will still surface
        # their own database errors when actually used.
        db.session.rollback()
        print("[E2] current_inst database lookup failed:", repr(exc))
        return None
...
def inject():
    try:
        ...
        db.session.rollback()
        print("[E2] context processor database error:", repr(exc))
        inst = None
    return {
        "inst": inst,
        "year": datetime.now().year,
        "e2_contact": "+923010012627",
        "is_master": session.get("role") == "superadmin",
    }
...
def handle_unexpected_error(exc):
    # Keep public login usable if an environment/database/template issue
    # escapes a route-level handler. Never expose secrets or a full traceback.
    print("[E2] Unhandled application error:", repr(exc))
    try:
...
.card{width:min(440px,92vw);background:#fff;padding:28px;border-radius:18px;box-shadow:0 12px 40px rgba(0,0,0,.10)}
h1{margin:0 0 6px}.muted{color:#667085}.msg{padding:10px;border-radius:8px;background:#fff1f0;color:#b42318;margin:16px 0}
a{color:#2457d6}
</style></head><body><div class="card">
<h1>E-2 School ERP</h1>
<p class="muted">Administrator Login</p>
<div class="msg">The login service could not load its data store. Please configure the persistent database and redeploy.</div>
<p class="muted">Health check: <a href="/healthz">/healthz</a></p>
</div></body></html>
"""), 200
    except Exception:
        return ("E-2 School ERP: login service unavailable", 503)
...
def master():
    ...
    if request.method=="POST":
        try:
            ...
            db.session.add(User(institute_id=i.id,email=email,password_hash=generate_password_hash(password),role="school_admin"))
            db.session.commit(); flash("New school created successfully.","success")
        except Exception:
...
def dashboard():
    ...
    fee_rate=round((collected/(collected+due_amount))*100,1) if (collected+due_amount)>0 else 0
...
SECURITY_DIR=DATA_DIR/"instance"
SECURITY_DIR.mkdir(parents=True,exist_ok=True)
MASTER_KEY_FILE=SECURITY_DIR/"e2_master.key"

def _master_key():
    # In serverless deployments the filesystem is ephemeral, so derive the
    # master key from the configured deployment secret when available.
    configured = os.environ.get("E2_SECRET_KEY") or os.environ.get("SECRET_KEY")
    if configured:
        return hashlib.sha256(configured.encode("utf-8")).digest()

    # Local/offline fallback: persist a random key in the writable data dir.
...
def migrate_erp22_content_bank():
    ...
    try:
        ...
        db.session.rollback()
        print("[ERP22.2] Migration failed:",exc)
        raise
...
# Recover an older local DB only for the local/offline build. Never copy a local DB into cloud Postgres.
if not DATABASE_URL:
...
def open_browser():
    if os.environ.get("E2_NO_BROWSER") == "1" or os.environ.get("E2_ENV", "development").lower() == "production":
        return
